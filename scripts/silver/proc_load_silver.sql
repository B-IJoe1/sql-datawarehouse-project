...
BEGIN 
    ...
    FROM (
        SELECT
            ...
            bdate,
            gen,
            UPPER(SUBSTRING(gen, PATINDEX('%[A-Za-z]%', gen), 1)) AS letter
    ...
    SELECT 
    REPLACE (cid, '-', ' ') cid, 
    CASE WHEN clean_cntry IS NULL THEN 'n/a'
    ...
    FROM (     
         SELECT     
    cid,     
    NULLIF( TRIM( REPLACE(cntry, CHAR(13), '')               
      	), ''         
     ) AS clean_cntry     
    ...
    SET @batch_end_time = GETDATE();
     PRINT '=========================================='; 
    PRINT 'Loading Silver Layer is Completed'; PRINT '   - Total Load Duration: ' + CAST(DATEDIFF(SECOND, @batch_start_time, @batch_end_time) AS NVARCHAR) + ' seconds';
     PRINT '==========================================';
